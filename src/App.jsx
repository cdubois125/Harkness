...
const fldStyle = {
  ...
  fontSize: 16, // iOS Safari auto-zooms on focus for any input under 16px — this is the fix, not a design choice
  color: INK,
  background: "#fff",
  outline: "none",
};
            ...
            <div className="mb-5">
              ...
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-sm" style={{ background: "#fff", border: `1px solid ${PARCHMENT_DEEP}` }}>
                ...
                <input
                  ...
                  style={{ fontFamily: "Source Serif 4, serif", fontSize: 16, color: INK }}
                />
              </div>
            </div>
            <div className="mb-2">
        ...
        <div className="space-y-2">
          {filtered.slice(0, 60).map((p) => (
            <div
              ...
              style={{ padding: "14px 18px", border: `1px solid ${PARCHMENT_DEEP}`, background: "#fff", boxShadow: SOFT_SHADOW }}
            >
              <div className="flex items-start gap-3">
                <Initials first={p.firstName} last={p.lastName} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between flex-wrap gap-x-3">
                    <div style={{ fontFamily: "Source Serif 4, serif", fontSize: 15, color: INK }}>{p.firstName} {p.lastName}</div>
                    <span
                      className="px-2 py-0.5 rounded-sm"
                      style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 10, letterSpacing: "0.06em", color: ROYAL, background: `${ROYAL}15` }}
                    >
                      {p.role.toUpperCase()}
                    </span>
                  </div>
                  {/* Wraps freely at any width — this is what replaces the old 9-column table that needed horizontal scrolling */}
                  <div className="flex items-center flex-wrap gap-x-4 gap-y-1 mt-1.5" style={{ fontFamily: "Source Serif 4, serif", fontSize: 13, color: SLATE }}>
                    {formatLocation(p) && (
                      <span className="flex items-center gap-1"><MapPin size={11} /> {formatLocation(p)}</span>
                    )}
                    {p.occupation && (
                      <span className="flex items-center gap-1"><Briefcase size={11} /> {p.occupation}{p.company ? ` · ${p.company}` : ""}</span>
                    )}
                    {p.field && (
                      <span>{p.field}{p.subfield ? ` · ${p.subfield}` : ""}</span>
                    )}
                    {showGradYear && p.gradYear && (
                      <span style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 11 }}>Class of {p.gradYear}</span>
                    )}
                    {showKidGrade && formatChildrenSummary(p) && (
                      <span style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 11 }}>Son's Class of {formatChildrenSummary(p)}</span>
                    )}
                    {(p.highSchool || p.college) && (
                      <span>{[p.highSchool, p.college].filter(Boolean).join(" → ")}</span>
                    )}
                  </div>
                </div>
                <ChevronRight size={16} color={BRASS} className="flex-shrink-0 mt-1" />
              </div>
            </div>
          ))}
        </div>
                    ...
                    <div key={year} className="rounded-xl overflow-hidden" style={{ border: `1px solid ${PARCHMENT_DEEP}`, background: "#fff", boxShadow: SOFT_SHADOW }}>
                      ...
                      {grads.map((p) => (
                        ...
                        >
                          <Initials first={p.firstName} last={p.lastName} />
                          <div className="flex-1">
                            <div style={{ fontFamily: "Source Serif 4, serif", fontSize: 16, color: INK }}>{p.firstName} {p.lastName}</div>
